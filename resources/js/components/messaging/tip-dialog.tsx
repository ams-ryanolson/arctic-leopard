...
type TipDialogProps = {
    ...
    onConfirm: (amount: number, mode: 'send' | 'request', paymentMethod?: string) => Promise<void>;
...
export default function TipDialog({
    ...
    const handleConfirm = useCallback(async () => {
        ...
        try {
            await onConfirm(parsedTipAmount, tipMode, tipMode === 'send' ? selectedPaymentMethod : undefined);
    ...
    }, [isValidTipAmount, parsedTipAmount, tipMode, selectedPaymentMethod, onConfirm, handleOpenChange]);
                ...
                <div className="space-y-5">
                    ...
                    {isSendMode ? (
                        ...
                        <p className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-xs text-white/60">
                            They'll get a notification right away. Once accepted, the request will appear in your chat
                            history.
                        </p>
                    )}
                    {tipError ? (
