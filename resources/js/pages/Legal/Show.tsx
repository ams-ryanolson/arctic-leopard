...
export default function LegalShow({ title, content, updatedAt }: { title: string; content: string; updatedAt?: string | null }) {
    ...
    useEffect(() => {
        ...
        nodes.forEach((node) => {
            ...
            const base = text.toLowerCase().trim().replace(/[^\w\s-]/g, '').replace(/\s+/g, '-');
        ...
        <LegalLayout title={title}>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-[0_24px_60px_-35px_rgba(249,115,22,0.45)] backdrop-blur">
                ...
                <p className="mt-2 text-sm text-white/65">
                    {updatedAt ? `Last updated ${updatedAt}.` : 'Updated content below.'} For questions, contact support.
                </p>
            </div>
            <div className="mt-8 grid gap-8 md:grid-cols-[minmax(0,1fr)_320px]">
                <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/7">
                    <article
                        ...
                        className="prose prose-invert mx-auto max-w-3xl p-6 sm:p-8 md:p-10
                                   prose-headings:text-white prose-h1:mt-0
                                   ...
                                   prose-h3:text-2xl sm:prose-h3:text-3xl prose-h3:mt-6 prose-h3:mb-3
                                   prose-p:text-white/85 prose-p:mt-4 prose-p:mb-5 leading-8 text-[15px] sm:text-base md:text-[17px]
                                   prose-ul:mt-4 prose-ul:mb-5 prose-li:text-white/80 prose-li:my-1.5
                ...
                <aside className="sticky top-24 h-fit rounded-2xl border border-white/10 bg-white/5 p-5">
                    <p className="text-xs font-semibold uppercase tracking-[0.3em] text-white/60">On this page</p>
                    <nav className="mt-3 space-y-2">
                        {headings.length === 0 && (
                            <p className="text-xs text-white/50">No sections detected.</p>
                        )}
                        {headings.map((h) => (
                            <a
                                key={h.id}
