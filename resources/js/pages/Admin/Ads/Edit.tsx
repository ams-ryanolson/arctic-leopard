...
export default function AdminAdsEdit({ ad }: AdminAdsEditProps) {
    const [isAdminAd, setIsAdminAd] = useState(
        !ad.budget_amount || ad.budget_amount === 0,
    );

    // Debug: Log the ad data to see what we're receiving (only once on mount)
    useEffect(() => {
        console.log('Ad data received:', {
            id: ad.id,
            name: ad.name,
            creativesCount: ad.creatives?.length || 0,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        console.log('Submitting ad update form', { data, isAdminAd });
        ...
        put(`/admin/ads/${ad.id}`, {
            ...
            onError: (errors) => {
                console.error('Ad update errors:', errors);
            },
            onFinish: () => {
                console.log('Ad update request finished');
            },
        });
    };
