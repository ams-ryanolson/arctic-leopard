...
export default function AnnouncementBar(propsIn?: {
	...
	const baseAnn: Announcement = props.announcements?.announcement ?? { enabled: false, level: 'info', message: '', dismissible: true };
	const baseMaint: Maintenance = props.announcements?.maintenance ?? { enabled: false, message: '' };
	const baseEmerg: Emergency = props.announcements?.emergency ?? { enabled: false, message: '' };

	// Admin detection (role name contains 'admin')
	const isAdmin =
		Boolean((props as any)?.auth?.user) &&
		Array.isArray((props as any)?.auth?.user?.roles) &&
		((props as any).auth.user.roles as any[]).some((r) => String(r?.name || '').toLowerCase().includes('admin'));

	const ann: Announcement = { ...baseAnn, ...(propsIn?.preview?.announcement ?? {}) };
	const maintenance: Maintenance = { ...baseMaint, ...(propsIn?.preview?.maintenance ?? {}) };
	const emergency: Emergency = { ...baseEmerg, ...(propsIn?.preview?.emergency ?? {}) };

	const [dismissedKey, setDismissedKey] = useState<string | null>(null);
	const [isVisible, setIsVisible] = useState(false);
	const [adminDismissKeys, setAdminDismissKeys] = useState<{ emer?: string; maint?: string; ann?: string }>({});
	...
	useEffect(() => {
		...
		setAdminDismissKeys({
			emer: emergency.message ? `rk_admin_dismiss_emergency_${btoa((emergency.message || '').slice(0, 48))}` : undefined,
			maint: maintenance.message ? `rk_admin_dismiss_maintenance_${btoa((maintenance.message || '').slice(0, 48))}` : undefined,
			ann: ann.message ? `rk_admin_dismiss_announcement_${btoa((ann.message || '').slice(0, 48))}` : undefined,
		});
	...
	const dismissed = propsIn?.isPreview ? false : dismissedKey ? localStorage.getItem(dismissedKey) === '1' : false;
	const adminDismissedEmer = propsIn?.isPreview ? false : adminDismissKeys.emer ? localStorage.getItem(adminDismissKeys.emer) === '1' : false;
	const adminDismissedMaint = propsIn?.isPreview ? false : adminDismissKeys.maint ? localStorage.getItem(adminDismissKeys.maint) === '1' : false;
	const adminDismissedAnn = propsIn?.isPreview ? false : adminDismissKeys.ann ? localStorage.getItem(adminDismissKeys.ann) === '1' : false;
	...
	const badge = (label: string, variant: 'info' | 'warn' | 'urgent' | 'maintenance' | 'emergency') => {
		const styles = {
			...
			maintenance: 'bg-gradient-to-br from-amber-500/20 to-orange-600/10 border-amber-400/30 text-amber-100 shadow-[0_0_20px_-5px_rgba(251,191,36,0.5)]',
			emergency: 'bg-gradient-to-br from-red-500/20 to-rose-600/10 border-red-400/30 text-red-100 shadow-[0_0_20px_-5px_rgba(239,68,68,0.5)]',
		};
	...
	const iconContainer = (children: React.ReactNode, variant: 'info' | 'warn' | 'urgent' | 'maintenance' | 'emergency') => {
		const styles = {
			...
			maintenance: 'bg-gradient-to-br from-amber-500/15 to-orange-600/5 border-amber-400/25 text-amber-200 shadow-[0_0_30px_-10px_rgba(251,191,36,0.6)]',
			emergency: 'bg-gradient-to-br from-red-500/15 to-rose-600/5 border-red-400/25 text-red-200 shadow-[0_0_30px_-10px_rgba(239,68,68,0.6)]',
		};

		return (
			<div className={`
				flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border-2
				backdrop-blur-sm transition-all duration-500 hover:scale-110 hover:rotate-6
				${styles[variant]}
			`}>
	...
	const card = (
		...
		return (
			...
			>
				<div 
					className={`
						w-full max-w-2xl overflow-hidden rounded-3xl border-2
						bg-gradient-to-br from-neutral-900/95 via-neutral-950/90 to-black/95
				...
				>
						...
						<div className="flex items-start gap-5">
							...
							{propsIn?.isPreview && (
								<button
									...
									className="group -mt-1 -mr-1 rounded-xl border border-white/10 bg-white/5 p-2.5 text-white/50 transition-all hover:bg-white/10 hover:text-white hover:border-white/20"
									aria-label="Close"
								>
					...
					<div className={`
						...
						${variant === 'urgent' ? 'text-red-500' : ''}
						${variant === 'maintenance' ? 'text-amber-500' : ''}
						${variant === 'emergency' ? 'text-red-500' : ''}
					`}></div>
				</div>
			</div>
		);
	...
	const panel = (
		...
		const barStyles = {
			...
			maintenance: 'border-amber-500/30 bg-gradient-to-r from-amber-950/40 via-orange-900/20 to-amber-950/40 shadow-[0_0_40px_-15px_rgba(251,191,36,0.6)]',
			emergency: 'border-red-500/30 bg-gradient-to-r from-red-950/40 via-rose-900/20 to-red-950/40 shadow-[0_0_40px_-15px_rgba(239,68,68,0.7)]',
		};

		return (
			<div 
				className={`
					relative z-50 w-full border-b border-white/10 
			...
			>
				...
				<div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 md:px-8">
					<div className={`
						flex items-center gap-4 rounded-2xl border-2 px-6 py-4 
						backdrop-blur-sm ring-1 ring-white/5
						transition-all duration-300 hover:shadow-2xl
						${barStyles[variant]}
					`}>
						{badge(badgeLabel, variant)}
						<span className="flex-1 text-sm font-medium leading-relaxed text-white/90 sm:text-base md:text-lg">
							{message}
						</span>
	...
	// Enhanced CTA button
	const ctaButton = (label: string, href?: string, onClick?: () => void, variant: 'primary' | 'secondary' = 'primary') => {
		...
		if (href) {
			return (
				<a
					...
					onClick={propsIn?.isPreview ? (e) => e.preventDefault() : onClick}
	...
	const renderBanner = () => {
		...
		if (ann.enabled && withinSchedule && ann.message && (!dismissed || !ann.dismissible)) {
			const level = ann.level as 'info' | 'warn' | 'urgent';
			const icons = { info: '📢', warn: '⚡', urgent: '🚨' };
			const headings = { info: 'Site Announcement', warn: 'Important Notice', urgent: 'Urgent Alert' };
			const subLabels = { info: 'Stay informed', warn: 'Please review', urgent: 'Immediate attention required' };
