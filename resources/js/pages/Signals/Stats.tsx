...
export default function SignalsStatsPage({
    ...
    const requiresActionShare =
        payoutSummary.pendingTransfers.filter(
            (transfer) => transfer.status === 'requires-action',
        ).length / Math.max(payoutSummary.pendingTransfers.length, 1);
    const payoutHealthScore = Math.max(
        35,
        Math.round(
            100 -
                requiresActionShare * 40 -
                (payoutSummary.accountHealth.status === 'attention' ? 20 : 0),
        ),
    );
    ...
    const headerQuickActions: HeaderQuickAction[] = recommendedAutomations
        .slice(0, 3)
        .map((automation) => ({
            id: automation.id,
            ...
            <div className="space-y-8 text-white">
                    ...
                    {metrics.map((metric) => (
                        ...
                        >
                            <CardHeader className="space-y-2">
                                <CardDescription className="text-xs tracking-[0.35em] text-white/50 uppercase">
                                    {metric.label}
                                </CardDescription>
                ...
                <section className="grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
                        ...
                        <Card className="border-white/10 bg-white/5">
                            <CardHeader className="pb-4">
                                ...
                                <CardDescription className="text-white/60">
                                    Priority alerts from monetization,
                                    moderation, and compliance pipelines.
                                </CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                {alerts.map((alert) => (
                                    ...
                                    >
                                        <div className="flex items-center justify-between gap-2 text-[0.6rem] tracking-[0.35em] uppercase">
                                            <span>
                                                {formatRelativeTime(
                                                    alert.timestamp,
                                                )}
                        ...
                        <Card className="border-white/10 bg-white/5">
                            <CardHeader className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                                <div>
                                    ...
                                    <CardDescription className="text-white/60">
                                        Real-time conversion velocity across
                                        tips and subscriptions.
                                    </CardDescription>
                                </div>
                                {monetizationTimeline.annotations.map(
                                    (annotation) => (
                                        <Badge
                                            key={annotation.label}
                                            className="rounded-full border-amber-400/40 bg-amber-400/10 text-[0.65rem] tracking-[0.3em] text-amber-100 uppercase"
                                        >
                                            {annotation.label}
                                        </Badge>
                                    ),
                    ...
                    <div className="space-y-6">
                        <Card className="border-white/10 bg-white/5">
                            <CardHeader className="pb-4">
                                <CardTitle className="text-xl font-semibold">
                                    Consent & safety queue
                                </CardTitle>
                                <CardDescription className="text-white/60">
                                    Every flag with time-sensitive follow-up and
                                    assigned owner.
                                </CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4">
                        ...
                        <Card className="border-white/10 bg-white/5">
                            <CardHeader className="pb-4">
                                ...
                                <CardDescription className="text-white/60">
                                    Upcoming transfers, treasury tasks, and
                            ...
                            <CardContent className="space-y-4">
                                <div className="grid gap-4 rounded-2xl border border-white/10 bg-black/30 p-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
                                    <div>
                                        ...
                                        <h3 className="mt-2 text-2xl font-semibold text-white">
                                            {formatCurrency(
                                                payoutSummary.nextPayoutAmount,
                                            )}
                                        </h3>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="space-y-2">
                                        {payoutSummary.pendingTransfers.map(
                                            (transfer) => (
                                                <div
                                                    key={transfer.label}
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <Badge
                                                            variant="outline"
                                                            className={cn(
                                                                'rounded-full border-white/15 bg-white/5 text-[0.65rem] tracking-[0.3em] uppercase',
                                                                transfer.status ===
                                                                    'requires-action'
                                ...
                                <div className="space-y-2">
                                    ...
                                    <ul className="space-y-2 text-sm text-white/70">
                                        {payoutSummary.accountHealth.messages.map(
                                            (message) => (
                                                <li
                                                    key={message}
                ...
                <section className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_35px_85px_-50px_rgba(249,115,22,0.6)]">
                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        <div>
                            ...
                            <p className="text-sm text-white/70">
                                Queue high-impact flows to capitalize on the
                                live signals before momentum cools.
                            </p>
                        </div>
                    ...
                    <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                        {recommendedAutomations.map((automation) => (
                            ...
                            >
                                <div className="space-y-3">
                                    <Badge className="rounded-full border-white/10 bg-white/10 text-[0.65rem] tracking-[0.3em] text-white/60 uppercase">
                                        {automation.impact}
                                    </Badge>
                ...
                <section className="rounded-3xl border border-white/10 bg-white/5 p-6">
                    ...
                    <p className="mt-2 text-sm text-white/70">
                        Walk stakeholders through the live operations arc
                        without leaving Signals.
                    </p>
                    <ol className="mt-4 space-y-3 text-sm text-white/75">
                        <li className="flex items-start gap-2">
                            ...
                            Start in the alert rail to acknowledge the Breath
                            Control Lab escalation and open the compliance
                            console.
                        </li>
                        <li className="flex items-start gap-2">
                            <span className="mt-0.5 inline-flex size-5 items-center justify-center rounded-full border border-white/15 bg-white/10 text-xs font-semibold">
                            ...
                            Pivot into Monetization → tip train surge and launch
                            the suggested thank-you loop automation.
                        </li>
                        <li className="flex items-start gap-2">
                            <span className="mt-0.5 inline-flex size-5 items-center justify-center rounded-full border border-white/15 bg-white/10 text-xs font-semibold">
                                3
                            </span>
                            Wrap in Tasks & Playbooks to assign the moderator
                            follow-up and confirm payout readiness on treasury.
                        </li>
                    </ol>
                </section>
            </div>
