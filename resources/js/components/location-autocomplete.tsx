            ...
            try {
                ...
                setResults(
                    data
                            ...
                            if (!finalCity && item.display_name) {
                                ...
                                for (let i = parts.length - 2; i >= 0; i--) {
                                    ...
                                    if (
                                        ...
                                        part.length > 2 &&
                                        !part.match(
                                            /^(street|st|avenue|ave|road|rd|boulevard|blvd|drive|dr|way|lane|ln)/i,
                                        )
                                    ) {
                                        finalCity = part;
                                        break;
                        ...
                        .filter(
                            (value): value is LocationSuggestion =>
                                value !== null,
                        ),
                );
            } catch (error_) {
        ...
        <div className="relative">
            <Input
                ...
                className={cn(
                    'pr-14',
                    autoActive &&
                        'border-white/35 shadow-[0_18px_45px_-30px_rgba(16,185,129,0.45)] ring-4 ring-emerald-500/20',
            ...
            <div className="pointer-events-none absolute top-1/2 right-3 flex -translate-y-1/2 items-center justify-center">
                {status === 'locating' ? (
                    <Spinner
                        className={cn(
                            'size-4',
                            autoActive ? 'text-emerald-300' : 'text-white/60',
                        )}
                    />
                ) : (
                    <span
                    ...
                    >
                        <MapPin
                            className={cn(
                                'size-4',
                                autoActive ? 'text-black' : undefined,
                            )}
                        />
                    </span>
                )}
                ...
                <div className="absolute z-50 mt-2 w-full overflow-hidden rounded-xl border border-white/10 bg-black/80 backdrop-blur">
                    ...
                    {!isLoading && results.length > 0 && (
                        <ul
                            ...
                            className="max-h-64 overflow-y-auto text-sm text-white/80"
                        >
