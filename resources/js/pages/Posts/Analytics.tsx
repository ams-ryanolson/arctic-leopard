...
import { Activity, ArrowUpRight, CircleDashed, Eye, Gauge, MapPin, Radar, Sparkles, Users } from 'lucide-react';
...
import type { AnalyticsTimelinePoint, PostAnalyticsPageProps, RecentViewEvent } from '@/types/analytics';
        ...
        <AppLayout
            breadcrumbs={[
                ...
                ...(authorProfileHref && authorUsername
                    ...
                    : []),
                { title: 'Post analytics', href: postAnalyticsRoutes.show.url({ post: post.id }) },
            ]}
        >
            ...
            <div className="space-y-20 text-white">
                ...
                <section id="activity" className="space-y-8">
                    ...
                    />
                    <Deferred data="recentViews" fallback={<RecentViewsFallback />}>
                        <RecentActivity />
                    </Deferred>
                </section>
            </div>
...
function Hero({
    ...
    const refreshedAt = formatDistanceToNow(new Date(metrics.live.refreshed_at), { addSuffix: true });
    ...
    return (
            ...
            <div className="relative flex flex-col gap-10 p-10 lg:flex-row lg:items-start lg:justify-between">
                <div className="max-w-3xl space-y-6">
                    <div className="flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-[0.35em] text-white/60">
                        ...
                        <span className="text-white/40">{range.start} → {range.end}</span>
                    </div>

                    <div className="space-y-3">
                        <h1 className="text-4xl font-semibold leading-tight text-white md:text-5xl">{heading.title}</h1>
                        {heading.excerpt && <p className="text-base text-white/70">{heading.excerpt}</p>}
                    </div>

                    <div className="flex flex-wrap items-center gap-3 text-sm text-emerald-100">
                        <span className="inline-flex items-center gap-2 rounded-full border border-emerald-400/60 bg-emerald-500/20 px-4 py-2 text-xs uppercase tracking-[0.35em] text-emerald-100">
        ...
        <div className="w-full max-w-sm rounded-3xl border border-white/10 bg-white/10 p-6 backdrop-blur md:self-end">
                    <h2 className="text-sm font-semibold uppercase tracking-[0.3em] text-white/60">Refine window</h2>
                    <p className="mt-2 text-xs text-white/70">
                        Switch ranges to see how the post is pacing. All charts, breakdowns, and live metrics adapt instantly.
                    </p>
                    <RangeSelector postId={post.id} options={range.options} active={range.value} />
                </div>
            </div>
        </section>
    );
            ...
            <ul className="flex flex-wrap gap-2 text-sm text-white/60">
                {sections.map((section) => (
                    <li key={section.id}>
                        <a
                            href={`#${section.id}`}
                            className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 uppercase tracking-[0.25em] transition hover:border-white/50 hover:text-white focus:outline-none focus-visible:border-white/60 focus-visible:text-white"
                        >
            ...
            {metricCards.map((card) => (
                ...
                >
                    ...
                    <div className="mt-6 space-y-1">
                        <p className="text-xs uppercase tracking-[0.35em] text-white/50">{card.label}</p>
                        <p className="text-3xl font-semibold text-white">{card.value}</p>
                    </div>
                    <p className="mt-2 text-xs text-white/60">{card.caption}</p>
                    {card.delta !== null && (
                        <p
                            className={`mt-4 inline-flex items-center gap-1 rounded-full px-3 py-1 text-[0.65rem] uppercase tracking-[0.3em] ${
                                card.delta > 0
                        ...
                        >
                            {card.delta > 0 ? <ArrowUpRight className="size-3" /> : <CircleDashed className="size-3" />}
        ...
        <div className="grid gap-6 lg:grid-cols-[2fr,1fr]">
            <Card className="space-y-6 border border-white/10 bg-white/5 p-6">
                <header className="flex items-center justify-between gap-3">
                    <div>
                        <p className="text-xs uppercase tracking-[0.35em] text-white/50">Country mix</p>
                        <p className="text-sm text-white/60">Top regions attributed by combined profile + IP signals.</p>
                    </div>
                </header>
            ...
            <Card className="flex flex-col gap-5 border border-white/10 bg-white/5 p-6">
                <div className="space-y-2">
                    <p className="text-xs uppercase tracking-[0.35em] text-white/50">Signed-in reach</p>
                ...
                <div className="h-3 w-full rounded-full bg-white/10">
                    <div
                        ...
                        style={{ width: `${Math.max(8, authenticatedPercent * 100)}%` }}
                    />
                </div>
                ...
                <p className="text-xs text-white/40">
                    Encourage followers to sign in for tighter geo + conversion attribution. Guests are deduped via
                    fingerprint & session hash.
                </p>
            </Card>
        </div>
    );
    ...
    if (!countries || countries.data.length === 0) {
        return (
            <div className="rounded-2xl border border-dashed border-white/10 bg-white/5 p-6 text-sm text-white/60">
                Not enough attributed traffic yet. Encourage signed-in followers or share the permalink to build a signal.
            </div>
        );
    }

    const topFive = countries.data.slice(0, 5);
    const tailCount = countries.total - topFive.reduce((sum, item) => sum + item.views, 0);
        ...
        <div className="space-y-4">
                    ...
                    >
                        <div className="flex items-start justify-between gap-4">
                            <div className="space-y-1">
                                <p className="text-sm font-semibold text-white">
                                    {flagForCountry(country.country_code)} {country.country}
                                </p>
                                <p className="text-xs uppercase tracking-[0.3em] text-white/50">{country.country_code}</p>
                            </div>
                            <div className="text-right">
                                <p className="text-sm font-medium text-white">{country.views.toLocaleString()}</p>
                        ...
                        <div className="mt-3 h-2 rounded-full bg-white/10">
                            <div
                                ...
                                style={{ width: `${Math.max(8, country.percentage * 100)}%` }}
                            />
                        </div>
                    </li>
                ))}
            ...
            {tailCount > 0 && (
                <p className="rounded-2xl border border-white/10 bg-white/5 p-4 text-xs text-white/50">
                    +{tailCount.toLocaleString()} views across additional regions
                </p>
            )}
        </div>
    );
        ...
        <Card className="space-y-8 overflow-hidden border border-white/10 bg-white/5 p-6">
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-white/60">Views and unique reach with live blending for today.</p>
                </div>
                <span className="inline-flex items-center gap-2 rounded-full border border-emerald-400/40 bg-emerald-500/10 px-3 py-1 text-xs uppercase tracking-[0.3em] text-emerald-200">
            ...
            <div className="overflow-x-auto rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                <DualLineChart series={timelineSeries} width={640} height={260} />
            </div>

            <div className="grid gap-3 md:grid-cols-3">
                {timeline.map((point) => (
                    <div key={point.date} className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-white/70">
                        <p className="text-xs uppercase tracking-[0.3em] text-white/40">{point.date}</p>
                        <p className="mt-2 text-white">
                            {point.views.toLocaleString()} views · {point.unique_viewers.toLocaleString()} unique
                        </p>
                        <p className="text-xs text-white/40">
                            {point.unique_authenticated_viewers.toLocaleString()} logged in · {point.unique_guest_viewers.toLocaleString()} guests
                        </p>
                    </div>
                ))}
            </div>
    ...
    if (!recentViews || recentViews.length === 0) {
        return (
            <Card className="rounded-2xl border border-dashed border-white/10 bg-white/5 p-8 text-sm text-white/60">
                No recent view events. Once viewers land on this post you’ll see live context, device origin, and surface details here.
            </Card>
        );
    }
    ...
    return (
        <div className="grid gap-3 md:grid-cols-2">
            {recentViews.map((event, index) => (
                <RecentViewCard key={`${event.occurred_at ?? 'unknown'}-${index}`} event={event} />
            ))}
        </div>
    );
}
        ...
        <article className="group relative overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-5 transition hover:border-amber-300/40 hover:bg-white/10">
            ...
            <div className="relative flex items-start gap-4">
                <Avatar className="size-12 border border-white/10">
                    <AvatarImage src={viewer?.avatar_url ?? undefined} alt={viewer?.display_name ?? undefined} />
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {viewer?.username && (
                            <Link
                                ...
                                prefetch
                                className="text-xs uppercase tracking-[0.3em] text-white/50 transition hover:text-white focus:outline-none focus-visible:text-white"
                            >
                                @{viewer.username}
                            </Link>
                        )}
                        {event.country_code && (
                            <span className="inline-flex items-center gap-1 rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[0.6rem] uppercase tracking-[0.3em] text-white/50">
...
function RangeSelector({
    ...
    return (
        <div className="mt-4 flex flex-wrap items-center gap-2">
            {options.map((option) => {
                const href = postAnalyticsRoutes.show.url({ post: postId }, { query: { range: option.value } });

                if (option.value === active) {
                    return (
                        <span
                            key={option.value}
                            className="rounded-full border border-amber-400/50 bg-amber-400/15 px-3 py-1 text-xs uppercase tracking-[0.3em] text-amber-100"
                        >
                            {option.label}
                        </span>
                    );
                ...
                return (
                    <Button
                        ...
                        size="sm"
                        asChild
                        className="rounded-full border border-white/15 px-3 py-1 text-xs uppercase tracking-[0.3em] text-white/60 hover:border-white/50 hover:text-white"
                    >
...
function buildMetricCards(timeline: AnalyticsTimelinePoint[], metrics: PageProps['metrics']) {
    ...
    const [purchasesDelta, purchasesDeltaPercent] = computeDelta(purchasesSpark);
...
function Sparkline({ values }: { values: number[] }) {
    ...
    return (
        <svg viewBox={`0 0 ${width} ${height}`} className="h-10 w-28 text-amber-300" role="img" aria-hidden>
            <path
                d={`${pathData}`}
                fill="none"
                stroke="currentColor"
...
function buildTimelineSeries(
    timeline: AnalyticsTimelinePoint[],
    datasets: Array<{ key: keyof AnalyticsTimelinePoint; label: string; color?: string }>,
) {
...
function formatDeltaLabel(delta: number, percent: number) {
    ...
    const percentLabel = percent === 0 ? '' : ` (${(percent * 100).toFixed(1)}%)`;
