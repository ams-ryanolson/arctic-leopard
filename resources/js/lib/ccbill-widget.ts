...
const WIDGET_SCRIPT_URL =
    import.meta.env.VITE_CCBILL_WIDGET_URL ||
    'https://js.ccbill.com/v1.13.1/ccbill-advanced-widget.js';
    ...
    return new Promise((resolve, reject) => {
        ...
        script.onerror = (error) => {
            ...
            console.error('Failed to load CCBill widget script:', {
                ...
                url: failedUrl,
                message:
                    'The widget script URL may be incorrect. Please check CCBill documentation for the correct widget script URL.',
            });
            reject(
                new Error(
                    `Failed to load CCBill widget script from ${failedUrl}. Please verify the script URL in CCBill documentation or contact CCBill support.`,
                ),
            );
        };
...
export async function initializeCCBillWidget(
    ...
    console.log('Checking for CCBill widget:', {
        ...
        config: {
            ...config,
            applicationId: config.applicationId ? '***' : undefined,
        },
    });

    if (!ccbillWidget) {
        const ccbillKeys = Object.keys(window).filter((k) =>
            k.toLowerCase().includes('ccbill'),
        );
        console.error(
            'CCBill object not found on window. Available CCBill-related keys:',
            ccbillKeys,
        );
        throw new Error(
            'CCBill widget library not loaded. Please check the script URL.',
        );
    }
    ...
    if (!AdvancedWidgetClass) {
        console.error(
            'CCBill.CCBillAdvancedWidget not found. Available CCBill keys:',
            Object.keys(ccbillWidget),
        );
        throw new Error(
            'CCBill Advanced Widget not available. Available keys: ' +
                Object.keys(ccbillWidget).join(', '),
        );
    }
    ...
    try {
        ...
        console.log(
            'Initializing CCBill widget with applicationId length:',
            config.applicationId?.length || 0,
        );
        console.log(
            'CCBill applicationId first 8 chars:',
            config.applicationId?.substring(0, 8) || 'missing',
        );
        ...
        const hasCreatePaymentToken3DS =
            typeof (widget as any).createPaymentToken3DS === 'function';
        const hasCreatePaymentToken =
            typeof (widget as any).createPaymentToken === 'function';
        console.log(
            'Widget has createPaymentToken3DS:',
            hasCreatePaymentToken3DS,
        );
...
function collectBrowserInfo(): Record<string, string> {
    return {
        ...
        browserHttpAcceptLanguate:
            navigator.language || navigator.languages?.[0] || 'en-US', // Note: CCBill docs show typo "Languate"
    };
}
...
export async function createPaymentToken3DS(
    ...
    if (!createTokenMethod || typeof createTokenMethod !== 'function') {
        console.error(
            'CCBill widget does not have createPaymentToken3DS method',
        );
        throw new Error(
            'CCBill widget does not have createPaymentToken3DS method',
        );
    ...
    try {
        ...
        requiredFields.forEach((fieldName) => {
            ...
            if (!element) {
                ...
                if (fieldName === 'cardNumber') {
                    ...
                    if (value.length !== 4) {
                        console.warn(
                            `expYear should be 4 digits (yyyy), got: ${value}`,
                        );
                    }
        ...
        if (stateElement && stateElement.value.trim()) {
            ...
            if (stateValue.length < 1 || stateValue.length > 3) {
                throw new Error(
                    'State/Province must be 1-3 characters if provided',
                );
            }
        }
        ...
        let threeDSRequestorURLField = document.querySelector(
            '[data-ccbill="threeDSRequestorURL"]',
        ) as HTMLInputElement;
        if (!threeDSRequestorURLField || !threeDSRequestorURLField.value) {
            if (form) {
                ...
                try {
                    ...
                    if (
                        ...
                        console.warn(
                            '⚠️ CCBill may reject HTTP .test domains for threeDSRequestorURL.',
                        );
                        console.warn(
                            'Consider using a production URL or contact CCBill support to whitelist your development domain.',
                        );
                ...
                threeDSRequestorURLField.setAttribute(
                    'data-ccbill',
                    'threeDSRequestorURL',
                );
            ...
            try {
                ...
                if (
                    ...
                    console.warn(
                        'threeDSRequestorURL converted to HTTPS:',
                        httpsUrl,
                    );
                }
            } catch (e) {
                console.error(
                    'Invalid threeDSRequestorURL format:',
                    existingUrl,
                    e,
                );
            }
        ...
        const browserFields = [
            ...
            {
                name: 'browserHttpUserAgent',
                value: browserInfo.browserHttpUserAgent,
            },
            { name: 'browserHttpAccept', value: browserInfo.browserHttpAccept },
            {
                name: 'browserHttpAcceptEncoding',
                value: browserInfo.browserHttpAcceptEncoding,
            },
            {
                name: 'browserHttpAcceptLanguate',
                value: browserInfo.browserHttpAcceptLanguate,
            }, // Note: CCBill docs show typo "Languate"
        ];
        ...
        Object.keys(fieldValues).forEach((fieldName) => {
            formFields[fieldName] =
                fieldName === 'cardNumber' || fieldName === 'cvv2'
                    ? '***'
                    : fieldValues[fieldName];
        });
        console.log(
            'Form field values before submission (trimmed):',
            formFields,
        );

        // Also log all data-ccbill fields from DOM to see what widget will read
        const allCcbillFields: Record<string, string> = {};
        const allCcbillFieldsDetailed: Record<
            string,
            { value: string; element: string; hasValue: boolean }
        > = {};
        ...
        console.log(
            'All data-ccbill fields detailed:',
            allCcbillFieldsDetailed,
        );

        // Verify all required fields have values
        const requiredFieldsFor3DS = [
            'firstName',
            'lastName',
            'email',
            'cardNumber',
        ...
        const missingRequiredFields = requiredFieldsFor3DS.filter(
            (fieldName) => {
                const element = document.querySelector(
                    `[data-ccbill="${fieldName}"]`,
                ) as HTMLInputElement | HTMLSelectElement;
        ...
        if (missingRequiredFields.length > 0) {
            console.error(
                'Missing required fields for createPaymentToken3DS:',
            ...
            throw new Error(
                `Missing required fields: ${missingRequiredFields.join(', ')}. Please ensure all required fields are filled.`,
            );
        }
        ...
        const numClientAccnum =
            typeof clientAccnum === 'string'
                ? parseInt(clientAccnum, 10)
                : clientAccnum;
        const numClientSubacc =
            typeof clientSubacc === 'string'
                ? parseInt(clientSubacc, 10)
                : clientSubacc;
        ...
        console.log(
            'Note: The widget reads form fields from DOM elements with data-ccbill attributes.',
        );
        ...
        const tokenId =
            result?.paymentToken?.paymentTokenId ||
        ...
        if (error?.response) {
            try {
                const responseData = await error.response
                    .json()
                    .catch(() => ({}));
                ccbillErrorMessage =
                ...
                try {
                    const responseText = await error.response
                        .text()
                        .catch(() => '');
                    console.error(
                        'CCBill API error response (text):',
                        responseText,
                    );
                    ccbillErrorMessage =
        ...
        if (!ccbillErrorMessage && error?.error) {
            ...
            if (nestedError?.response) {
                try {
                    const responseData = await nestedError.response
                        .json()
                        .catch(() => ({}));
                    ccbillErrorMessage =
                        responseData?.error?.message ||
                    ...
                    ccbillErrorDetails = responseData;
                    console.error(
                        'CCBill API error response (nested):',
                        responseData,
                    );
                } catch (e) {
                    try {
                        const responseText = await nestedError.response
                            .text()
                            .catch(() => '');
                        console.error(
                            'CCBill API error response (nested text):',
                            responseText,
                        );
                        ccbillErrorMessage =
                            responseText ||
                            nestedError.response.statusText ||
                            '';
                    } catch (e2) {
                        try {
                            ...
                            ccbillErrorMessage =
                                nestedError.response.statusText ||
                                `HTTP ${nestedError.response.status}`;
                        }
                    }
                }
        ...
        if (!ccbillErrorMessage && error?.status === 400) {
                ...
                try {
                    const bodyData =
                        typeof error.body === 'string'
                            ...
                            : error.body;
                    ccbillErrorMessage =
                        bodyData?.error?.message || bodyData?.message || '';
                    ccbillErrorCode =
                        bodyData?.error?.code || bodyData?.code || '';
                    ccbillErrorDetails = bodyData;
                    console.error(
                        'CCBill API error from error.body:',
                        bodyData,
                    );
                } catch (e) {
            ...
            if (!ccbillErrorMessage) {
                ccbillErrorMessage =
                    'Bad Request (400). Please check the Network tab in your browser DevTools (F12 → Network → find the failed request to api.ccbill.com → click it → check the Response tab) to see the detailed error response from CCBill.';
            }
        }
        ...
        const isCorsError =
            ...
            (errorError?.name === 'TypeError' &&
                errorErrorMessage.includes('fetch'));
        ...
        if (isCorsError) {
            ...
            const isTestDomain =
                hostname.endsWith('.test') || hostname.endsWith('.local');
            ...
            if (isLocalhost || isTestDomain) {
                const corsError = new Error(
                    'CORS Error: CCBill API does not allow requests from development domains (localhost or .test domains).\n\n' +
                        'Solutions:\n' +
                        '1. Contact CCBill support to whitelist your development domain: ' +
                ...
                const corsError = new Error(
                    'CORS Error: CCBill API is blocking requests from this origin. ' +
                        'Please contact CCBill support to whitelist your domain: ' +
                        window.location.origin,
                );
                (corsError as any).isCorsError = true;
                throw corsError;
        ...
        if (non3DSMethod && typeof non3DSMethod === 'function') {
            try {
                console.log(
                    'Attempting fallback to createPaymentToken (non-3DS)',
                );
                ...
                if (Object.keys(optionalParams).length > 0) {
                    result = await non3DSMethod.call(
                        widget,
                        authToken,
                        numClientAccnum,
                        numClientSubacc,
                ...
                console.log(
                    'createPaymentToken (non-3DS) raw response:',
                    result,
                );
                ...
                if (result instanceof Response) {
                    console.log(
                        'Response is a fetch Response object, parsing JSON...',
                    );
                ...
                const tokenId =
                    parsedResult?.paymentTokenId ||
                    parsedResult?.tokenId ||
                    parsedResult?.token;

                if (tokenId && typeof tokenId === 'string') {
                    console.log(
                        '✓ Successfully created payment token (non-3DS):',
                        tokenId,
                    );
                    return { tokenId, is3DS: false };
                } else {
                    console.error(
                        'Unexpected non-3DS token response:',
                        parsedResult,
                    );
                    console.error(
                        'Available keys:',
                    ...
                    throw new Error(
                        'Invalid token response from CCBill widget (non-3DS)',
                    );
                }
            } catch (fallbackError: any) {
                console.error(
                    'Fallback to non-3DS also failed:',
                    fallbackError,
                );
                ...
                if (
                    fallbackError?.errorCode === 'BE-123' ||
                    fallbackError?.message?.includes('Encryption')
                ) {
                    console.error(
                        '⚠️ Encryption Error (BE-123) - This usually means:',
                    );
                    console.error(
                        '1. The frontend bearer token is invalid or expired',
                    );
                    console.error(
                        '2. The widget was not initialized with the correct applicationId',
                    );
                    console.error(
                        '3. The widget cannot encrypt the payment data',
                    );
                    console.error(
                        'Full error object:',
                        JSON.stringify(
                ...
                if (fallbackError?.response) {
                    try {
                        const responseData = await fallbackError.response
                            .json()
                            .catch(() => ({}));
                        fallbackCcbillErrorMessage =
                            responseData?.error?.message ||
                            responseData?.message ||
                            '';
                        fallbackCcbillErrorCode =
                            responseData?.error?.code ||
                            responseData?.code ||
                            '';
                        fallbackCcbillErrorDetails = responseData;
                        console.error(
                            'CCBill API fallback error response:',
                            responseData,
                        );
                    } catch (e) {
                        try {
                            const responseText = await fallbackError.response
                                .text()
                                .catch(() => '');
                            console.error(
                                'CCBill API fallback error response (text):',
                                responseText,
                            );
                            fallbackCcbillErrorMessage =
                                responseText ||
                            ...
                            try {
                                ...
                                fallbackCcbillErrorMessage =
                                    fallbackError.response.statusText ||
                                    `HTTP ${fallbackError.response.status}`;
                            }
                        }
                    }
                } else if (fallbackError?.status === 400) {
                    fallbackCcbillErrorMessage =
                        'Bad Request (400). Please check that all required fields are filled correctly.';
                ...
                if (fallbackCcbillErrorMessage) {
                    ...
                    (userError as any).ccbillError = true;
                    (userError as any).ccbillErrorCode =
                        fallbackCcbillErrorCode;
                    (userError as any).ccbillErrorDetails =
                        fallbackCcbillErrorDetails;
                ...
                const fallbackErrorErrorMessage =
                    fallbackErrorError?.message || '';
                ...
                const isFallbackCorsError =
                    ...
                    (fallbackErrorError?.name === 'TypeError' &&
                        fallbackErrorErrorMessage.includes('fetch'));
                ...
                if (isFallbackCorsError) {
                    ...
                    if (isLocalhost || isTestDomain) {
                        ...
                        (corsError as any).isCorsError = true;
                        (corsError as any).isLocalhost =
                            isLocalhost || isTestDomain;
                        throw corsError;
                    }
                }
            }
