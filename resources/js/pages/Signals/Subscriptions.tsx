...
import { Calendar, FileSpreadsheet, Plus, ShieldAlert, TrendingDown, TrendingUp } from 'lucide-react';
...
const eventCopy: Record<SubscriptionEvent['type'], { label: string; tone: string }> = {
...
export default function SubscriptionsPage({
    ...
    const headerQuickActions: HeaderQuickAction[] = quickActions.slice(0, 2).map((action) => ({
        id: action.id,
        title: action.label,
        description: action.description,
        icon: action.severity === 'high' ? TrendingDown : action.severity === 'medium' ? TrendingUp : FileSpreadsheet,
        ...
        >
            ...
            <div className="space-y-8 text-white">
                <section className="grid gap-6 xl:grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)]">
                    <Card className="border-white/10 bg-white/5">
                        <CardHeader className="flex items-start justify-between gap-4">
                            <div>
                                <CardTitle className="text-2xl font-semibold">Plan catalog</CardTitle>
                                <CardDescription className="text-white/60">
                                    Manage active tiers, invite-only rituals, and draft offerings.
                                </CardDescription>
                            </div>
                                    ...
                                    <tbody className="divide-y divide-white/10 bg-black/20">
                                        {plans.map((plan) => (
                                            <tr key={plan.id}>
                                                ...
                                                <td className="px-4 py-3 text-white/70">{plan.price}</td>
                                                <td className="px-4 py-3 text-right text-white/70">
                                                    {plan.members.toLocaleString()}
                                                </td>
                                                <td className="px-4 py-3 text-white/60">{plan.visibility}</td>
                                                <td className="px-4 py-3">
                                                    <span
                                                        className={cn(
                                                            'inline-flex items-center rounded-full border px-2.5 py-1 text-[0.65rem] uppercase tracking-[0.3em]',
                    ...
                    <Card className="border-white/10 bg-white/5">
                        <CardHeader>
                            <CardTitle className="text-xl font-semibold">Plan builder</CardTitle>
                            <CardDescription className="text-white/60">
                                Preview the fields we’ll ask for when you create or update a tier.
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <form className="space-y-4 text-sm">
                                <div className="grid gap-3">
                                    <Label htmlFor="plan-name" className="text-xs uppercase tracking-[0.3em] text-white/50">
                                        Tier name
                                    </Label>
                                    <Input
                                        id="plan-name"
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="grid gap-3">
                                        <Label htmlFor="plan-price" className="text-xs uppercase tracking-[0.3em] text-white/50">
                                            Price ({planForm.defaults.currency})
                                        </Label>
                                        <Input
                                            id="plan-price"
                                            type="number"
                                            defaultValue={planForm.defaults.price}
                                            className="border-white/15 bg-black/30 text-white placeholder:text-white/40"
                                        />
                                    </div>
                                    <div className="grid gap-3">
                                        <Label htmlFor="plan-cadence" className="text-xs uppercase tracking-[0.3em] text-white/50">
                                            Billing cadence
                                        </Label>
                                        <select
                                            id="plan-cadence"
                                            defaultValue={planForm.defaults.billingCadence}
                                            className="h-10 rounded-md border border-white/15 bg-black/30 px-3 text-sm text-white"
                                        >
                                            {planForm.cadenceOptions.map((option) => (
                                ...
                                <div className="grid gap-3">
                                    <Label htmlFor="plan-visibility" className="text-xs uppercase tracking-[0.3em] text-white/50">
                                        Visibility
                                    </Label>
                                    <select
                                        id="plan-visibility"
                                        defaultValue={planForm.defaults.visibility}
                                        className="h-10 rounded-md border border-white/15 bg-black/30 px-3 text-sm text-white"
                                    >
                                        {planForm.visibilityOptions.map((option) => (
                                ...
                                <div className="grid gap-3">
                                    <Label htmlFor="plan-description" className="text-xs uppercase tracking-[0.3em] text-white/50">
                                        Description
                                    </Label>
                                    <textarea
                                        id="plan-description"
                                        defaultValue={planForm.defaults.description}
                ...
                <section className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
                    <Card className="border-white/10 bg-white/5">
                        <CardHeader>
                            <CardTitle className="text-2xl font-semibold">Cohort compasses</CardTitle>
                            <CardDescription className="text-white/60">
                                Track new joins, retention, and ARPU uplift per monthly cohort.
                            </CardDescription>
                        </CardHeader>
                            ...
                            {cohorts.map((cohort) => (
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        <p className="text-xs uppercase tracking-[0.35em] text-white/50">{cohort.month}</p>
                                        <Badge className="rounded-full border-emerald-400/35 bg-emerald-400/10 text-[0.65rem] uppercase tracking-[0.3em] text-emerald-100">
                                            {Math.round(cohort.retention * 100)}% retention
                                        </Badge>
                                    </div>
                                    <h3 className="mt-3 text-3xl font-semibold text-white">{cohort.newSubscribers.toLocaleString()}</h3>
                                    <p className="text-sm text-white/70">New subscribers</p>
                                    <div className="mt-4 flex items-center justify-between text-sm">
                                        <span className="text-white/70">ARPU</span>
                                        <span className="font-semibold text-white">{formatCurrency(cohort.arpu)}</span>
                                    </div>
                                </div>
                            ))}
                        </CardContent>
                    ...
                    <Card className="border-white/10 bg-white/5">
                        <CardHeader>
                            <CardTitle className="text-xl font-semibold">Churn funnel</CardTitle>
                            <CardDescription className="text-white/60">
                                Current journey to drop-off with real-time delta indicators.
                            </CardDescription>
                        </CardHeader>
                            ...
                            {churnFunnel.map((stage, index) => (
                                <div key={stage.stage}>
                                    <div className="flex items-center justify-between text-sm text-white/60">
                                        <span className="uppercase tracking-[0.25em] text-white/50">{stage.stage}</span>
                                    ...
                                    <div className="mt-2 h-3 overflow-hidden rounded-full bg-white/10">
                                        <div
                                            className={cn(
                                                ...
                                                index === 0 && 'bg-emerald-400/80',
                                                index === 1 && 'bg-amber-400/80',
                ...
                <section className="grid gap-6 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
                    <Card className="border-white/10 bg-white/5">
                        <CardHeader>
                            <CardTitle className="text-xl font-semibold">Recent subscription activity</CardTitle>
                            <CardDescription className="text-white/60">
                                Latest upgrades, downgrades, and new conversions flowing in.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-3">
                            {recentEvents.map((event) => (
                                ...
                                >
                                    <div>
                                        <p className={cn('font-semibold text-white', eventCopy[event.type].tone)}>
                                    ...
                                    <div className="text-right">
                                        <p className="text-sm font-semibold text-white">{formatCurrency(event.value)}</p>
                    ...
                    <Card className="border-white/10 bg-white/5">
                        <CardHeader>
                            ...
                            <CardDescription className="text-white/60">
                                High-leverage nudges generated from churn and cohort insights.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            {quickActions.map((action) => (
                                <ActionBanner
                                    ...
                                    title={action.label}
                                    description={action.description}
                                    tone={actionSeverityStyles[action.severity] as 'rose' | 'amber' | 'sky'}
                                    icon={action.severity === 'high' ? ShieldAlert : action.severity === 'medium' ? TrendingUp : FileSpreadsheet}
                                    href="/signals/settings"
