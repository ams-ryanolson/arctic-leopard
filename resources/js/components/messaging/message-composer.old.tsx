...
export default function MessageComposer({
    ...
    const startRecording = useCallback(
        async (event?: React.MouseEvent) => {
            ...
            try {
                ...
                console.log(
                    '🎤 [AUDIO] Setting up MediaRecorder with session ID',
                    {
                        currentSessionId,
                        mimeType,
                    },
                ...
                mediaRecorder.ondataavailable = (event) => {
                    ...
                    if (currentSessionId !== audioSessionIdRef.current) {
                        console.log(
                            '🎤 [AUDIO] ondataavailable IGNORED - session mismatch',
                            {
                    ...
                    if (event.data && event.data.size > 0) {
                        console.log(
                            '🎤 [AUDIO] ondataavailable - chunk received',
                            {
                ...
                mediaRecorder.onerror = (event) => {
                    ...
                    if (currentSessionId !== audioSessionIdRef.current) {
                        console.log(
                            '🎤 [AUDIO] onerror IGNORED - session mismatch',
                            {
                                handlerSessionId: currentSessionId,
                                currentSessionId: audioSessionIdRef.current,
                            },
                ...
                mediaRecorder.onstop = () => {
                    ...
                    if (currentSessionId !== audioSessionIdRef.current) {
                        console.log(
                            '🎤 [AUDIO] onstop IGNORED - session mismatch',
                            {
                                handlerSessionId: currentSessionId,
                                currentSessionId: audioSessionIdRef.current,
                    ...
                    console.log(
                        '🎤 [AUDIO] onstop PROCESSING - valid session',
                        {
                            sessionId: currentSessionId,
                            chunksCount: audioChunksRef.current.length,
                            chunksSize: audioChunksRef.current.reduce(
                    ...
                    setTimeout(() => {
                        ...
                        if (currentSessionId !== audioSessionIdRef.current) {
                            console.log(
                                '🎤 [AUDIO] onstop IGNORED - session changed during delay',
                                {
                                    handlerSessionId: currentSessionId,
                                    currentSessionId: audioSessionIdRef.current,
                                },
                        ...
                        if (audioChunksRef.current.length === 0) {
                            ...
                            if (recordingTime > 0) {
                                console.warn(
                                    '🎤 [AUDIO] No audio data recorded - showing error',
                                );
                                setError(
                                ...
                                console.log(
                                    '🎤 [AUDIO] No chunks but recordingTime is 0 - not showing error',
                                );
                            }
                            setIsRecordingAudio(false);
                            setWaveformData([]);
                ...
                try {
                    ...
                    console.log(
                        '🎤 [AUDIO] mediaRecorder.start() succeeded immediately',
                        {
                            sessionId: currentSessionId,
                            state: mediaRecorder.state,
                        },
                    ...
                    console.error(
                        '🎤 [AUDIO] mediaRecorder.start() threw error',
                        {
                            sessionId: currentSessionId,
                            error: startError,
                            state: mediaRecorder.state,
    ...
    const handleAudioButtonClick = useCallback(async () => {
        ...
        setTimeout(() => {
            console.log(
                '🎤 [AUDIO] setTimeout callback - calling startRecording',
                {
                    sessionId: audioSessionIdRef.current,
                    isRecordingAudio,
                },
    ...
    const startVideoRecording = useCallback(
        async (event?: React.MouseEvent) => {
            ...
            try {
                ...
                setTimeout(() => {
                    if (videoPreviewRef.current && stream) {
                        ...
                        videoPreviewRef.current
                            ...
                            .catch((error) => {
                                console.error(
                                    '📹 [VIDEO] Error playing video preview:',
                                    error,
                                );
                            });
                    }
                ...
                console.log(
                    '📹 [VIDEO] Setting up MediaRecorder with session ID',
                    {
                        currentSessionId,
                        mimeType,
                    },
                ...
                mediaRecorder.ondataavailable = (event) => {
                    ...
                    if (currentSessionId !== videoSessionIdRef.current) {
                        console.log(
                            '📹 [VIDEO] ondataavailable IGNORED - session mismatch',
                            {
                                handlerSessionId: currentSessionId,
                    ...
                    if (event.data && event.data.size > 0) {
                        console.log(
                            '📹 [VIDEO] ondataavailable - chunk received',
                            {
                                sessionId: currentSessionId,
                                size: event.data.size,
                ...
                mediaRecorder.onerror = (event) => {
                    ...
                    if (currentSessionId !== videoSessionIdRef.current) {
                        console.log(
                            '📹 [VIDEO] onerror IGNORED - session mismatch',
                            {
                                handlerSessionId: currentSessionId,
                                currentSessionId: videoSessionIdRef.current,
                            },
                ...
                mediaRecorder.onstop = () => {
                    // Check stream state when stop fires
                    const streamStillActive =
                        videoStreamRef.current?.active ?? false;
                    const tracksState =
                        videoStreamRef.current?.getTracks().map((t) => ({
                            kind: t.kind,
                            readyState: t.readyState,
                            enabled: t.enabled,
                            muted: t.muted,
                    ...
                    if (currentSessionId !== videoSessionIdRef.current) {
                        console.log(
                            '📹 [VIDEO] onstop IGNORED - session mismatch',
                            {
                                handlerSessionId: currentSessionId,
                                currentSessionId: videoSessionIdRef.current,
                                reason: 'Session ID changed - new recording started',
                    ...
                    console.log(
                        '📹 [VIDEO] onstop PROCESSING - valid session',
                        {
                            sessionId: currentSessionId,
                            chunksCount: videoChunksRef.current.length,
                            chunksSize: videoChunksRef.current.reduce(
                                (sum, chunk) => sum + chunk.size,
                                0,
                            ),
                            videoRecordingTime,
                    ...
                    setTimeout(() => {
                        ...
                        if (currentSessionId !== videoSessionIdRef.current) {
                            console.log(
                                '📹 [VIDEO] onstop IGNORED - session changed during delay',
                                {
                                    handlerSessionId: currentSessionId,
                                    currentSessionId: videoSessionIdRef.current,
                                },
                        ...
                        if (videoChunksRef.current.length === 0) {
                            ...
                            if (videoRecordingTime > 0) {
                                console.warn(
                                    '📹 [VIDEO] No video data recorded - showing error',
                                );
                                setError(
                                ...
                                console.log(
                                    '📹 [VIDEO] No chunks but videoRecordingTime is 0 - not showing error',
                                );
                            }
                            setIsRecordingVideo(false);
                            videoChunksRef.current = [];
                        ...
                        if (totalSize === 0) {
                            if (videoRecordingTime > 0) {
                                console.warn(
                                    '📹 [VIDEO] Video chunks have no data - showing error',
                                );
                                setError(
                        ...
                        if (blob.size === 0) {
                            if (videoRecordingTime > 0) {
                                console.warn(
                                    '📹 [VIDEO] Video blob is empty - showing error',
                                );
                                setError(
                                    'No video data was recorded. Please try again.',
                                );
                ...
                try {
                    ...
                    console.log(
                        '📹 [VIDEO] mediaRecorder.start() succeeded immediately',
                        {
                            sessionId: currentSessionId,
                            state: mediaRecorder.state,
                        },
                    ...
                    console.error(
                        '📹 [VIDEO] mediaRecorder.start() threw error',
                        {
                            sessionId: currentSessionId,
                            error: startError,
                            state: mediaRecorder.state,
    ...
    const handleVideoButtonClick = useCallback(async () => {
        ...
        setTimeout(() => {
            console.log(
                '📹 [VIDEO] setTimeout callback - calling startVideoRecording',
                {
                    sessionId: videoSessionIdRef.current,
                    isRecordingVideo,
                },
    ...
    useEffect(() => {
        return () => {
            console.log(
                '🧹 [CLEANUP] Component unmounting - cleaning up all resources',
                {
    ...
    useEffect(() => {
        if (
            ...
            console.log(
                '📹 [VIDEO] useEffect: Attaching stream to video element',
                {
                    streamActive: stream.active,
                    hasVideoTracks: stream.getVideoTracks().length > 0,
                    videoElement: !!video,
            ...
            video
                ...
                .then(() => {
                    console.log(
                        '📹 [VIDEO] useEffect: Video preview started playing',
                    );
                })
                .catch((error) => {
                    console.error(
                        '📹 [VIDEO] useEffect: Error playing video:',
                        error,
                    );
                });
            ...
            return () => {
                if (video.srcObject) {
                    console.log(
                        '📹 [VIDEO] useEffect cleanup: Clearing video srcObject',
                    );
                        ...
                        {showVideoRecorder ? (
                            ...
                            <div className="relative">
                                <textarea
                                    ...
                                    rows={4}
                                    className="h-28 w-full resize-none rounded-2xl border border-white/10 bg-white/5 px-3 py-3 pr-12 text-sm text-white/90 placeholder:text-white/40 focus:border-amber-400/50 focus:ring-2 focus:ring-amber-400/40 focus:outline-none sm:px-4 sm:pr-12"
                                />
                                <div className="absolute right-3 bottom-3">
                                    <button
                                        ...
                                        className="flex size-8 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white/70 transition hover:border-white/25 hover:bg-white/10 focus-visible:ring-2 focus-visible:ring-amber-400/40 focus-visible:outline-none"
                ...
                <DialogContent className="max-w-md border-white/10 bg-neutral-950/95 text-white">
                    <DialogHeader>
                        ...
                        <DialogDescription className="mt-2 text-xs text-white/50">
                            Send a quick tip or request one to keep the
