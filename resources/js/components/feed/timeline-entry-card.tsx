...
import { BarChart3, Bookmark as BookmarkIcon, BookmarkCheck } from 'lucide-react';
            ...
            (entries) => {
                for (const entry of entries) {
                    ...
                    if (entry.isIntersecting && entry.intersectionRatio >= 0.5) {
                        if (dwellTimerRef.current !== null && typeof window !== 'undefined') {
                    ...
                    } else if (dwellTimerRef.current !== null && typeof window !== 'undefined') {
                        window.clearTimeout(dwellTimerRef.current);
                        dwellTimerRef.current = null;
                    }
                }
    ...
    return (
        <div ref={cardWrapperRef}>
            <Card className="border border-white/10 bg-white/5 p-6 text-sm text-white/75">
                <header className="flex flex-wrap items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="flex flex-wrap items-center gap-2 text-xs uppercase tracking-[0.3em] text-white/50">
                ...
                {post.body && (
                    <p className="mt-4 whitespace-pre-line text-base text-white/80">{post.body}</p>
                )}
                ...
                {tipGoal && tipGoal.amount && tipGoal.currency && (
                    <div className="mt-4 rounded-2xl border border-emerald-400/40 bg-emerald-500/10 p-4 text-xs text-emerald-100">
                        <p className="text-[0.65rem] uppercase tracking-[0.35em] text-emerald-200/80">
                            Tip train goal
                        </p>
                        <div className="mt-2 flex flex-col gap-1 text-sm text-white">
                            <span className="font-semibold">
                                {formatCurrency(tipGoal.amount, tipGoal.currency)}
                            </span>
                            {tipGoal.label && <span className="text-white/80">{tipGoal.label}</span>}
                            {tipGoal.deadline && (
                                <span className="text-emerald-100/70">
                                    Ends {new Date(tipGoal.deadline).toLocaleString()}
                                </span>
                            )}
                        </div>
                    </div>
                ...
                <footer className="mt-6 flex flex-wrap gap-2 text-xs text-white/60">
                    ...
                    >
                        <span className="flex items-center gap-2">
                            <Spark sparked={hasLiked} className={cn('size-3.5', !hasLiked && 'text-white/60')} />
                    ...
                    {post.can?.viewAnalytics ? (
                        ...
                        <Button
                            ...
                            className={cn(
                                'rounded-full px-4 text-xs text-white/75 hover:bg-white/10 hover:text-white',
                                post.locked && 'border border-amber-400/30 bg-amber-400/10 text-amber-200',
                            )}
                            onClick={() => onPurchase?.(post.id)}
                            disabled={!post.locked || disabled}
                        >
                            {post.locked ? 'Unlock to view' : `Views (${viewsCount})`}
                        </Button>
                    )}
                </footer>
            </Card>
