...
import { CheckCircle2, Clock, FileText, XCircle, Home } from 'lucide-react';
...
export default function AppealShow() {
    ...
    const config = statusConfig[appeal.status as keyof typeof statusConfig] || statusConfig.pending;
                    ...
                    <Card className={`border ${config.borderColor} ${config.bgColor}`}>
                        ...
                        <CardContent className="space-y-6">
                            <div className="space-y-2">
                                <h3 className="text-sm font-semibold text-white">Appeal Type</h3>
                                <p className="text-sm text-white/70 capitalize">{appeal.appeal_type}</p>
                            </div>

                            <div className="space-y-2">
                                <h3 className="text-sm font-semibold text-white">Your Appeal</h3>
                                ...
                                <div className="space-y-4">
                                    ...
                                    {appeal.review_notes && (
                                        <Alert className={`${config.borderColor} ${config.bgColor}`}>
                                            ...
                                            <AlertTitle className="text-white">Review Notes</AlertTitle>
                                            <AlertDescription className="text-white/80 whitespace-pre-wrap">
                                                {appeal.review_notes}
                                            </AlertDescription>
                                        </Alert>
                                    )}
                                    ...
                                    {appeal.status === 'approved' && (
                                        <Alert className="border-green-500/30 bg-green-950/10">
                                            ...
                                            <AlertTitle className="text-white">Appeal Approved</AlertTitle>
                                            <AlertDescription className="text-white/80">
                                                {!props.user_status.is_suspended && !props.user_status.is_banned ? (
                                                    <>
                                                        Your appeal has been approved and your account has been restored. You can now return to using the site.
                                                    </>
                                                ) : (
                                                    <>
                                                        Your appeal has been approved. Your account status should be restored shortly.
                                                    </>
                                                )}
                                            </AlertDescription>
                                        </Alert>
                                    ...
                                    {appeal.status === 'rejected' && (
                                        <Alert className="border-red-500/30 bg-red-950/10">
                                            ...
                                            <AlertTitle className="text-white">Appeal Rejected</AlertTitle>
                                            <AlertDescription className="text-white/80">
                                                Your appeal has been reviewed and the decision to {appeal.appeal_type} your account has been upheld.
                                            </AlertDescription>
                                        </Alert>
                                    )}
                                </div>
                            ...
                            {appeal.status === 'pending' && (
                                <Alert className="border-amber-500/30 bg-amber-950/10">
                                    ...
                                    <AlertTitle className="text-white">Under Review</AlertTitle>
                                    <AlertDescription className="text-white/80">
                            ...
                            {appeal.status === 'approved' && !props.user_status.is_suspended && !props.user_status.is_banned && (
                                <div className="pt-4 border-t border-white/10">
                                    <Button
                                        onClick={() => router.visit('/')}
                                        className="w-full sm:w-auto bg-gradient-to-r from-green-500 to-emerald-600 text-white hover:from-green-600 hover:to-emerald-700"
                                    >
