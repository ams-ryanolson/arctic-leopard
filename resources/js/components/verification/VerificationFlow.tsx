...
export default function VerificationFlow({
        ...
        try {
            ...
            const errorMessage =
                caught instanceof Error
                    ? caught.message
                    : 'Failed to open verification window. Please try again.';
        ...
        return (
            <div className="rounded-3xl border border-emerald-400/20 bg-emerald-400/10 p-8">
                <div className="flex items-start gap-4">
                    <div className="flex size-12 items-center justify-center rounded-2xl border border-emerald-400/40 bg-emerald-400/20">
    ...
    if (
        verificationStatus?.status === 'renewal_required' ||
        verificationStatus?.needs_renewal
    ) {
        const gracePeriodEnd = verificationStatus.expires_at
            ? new Date(
                  new Date(verificationStatus.expires_at).getTime() +
                      30 * 24 * 60 * 60 * 1000,
              )
            : null;
        ...
        return (
            <div className="space-y-6">
                <div className="rounded-3xl border border-amber-400/20 bg-amber-400/10 p-8">
                    <div className="flex items-start gap-4">
                        <div className="flex size-12 items-center justify-center rounded-2xl border border-amber-400/40 bg-amber-400/20">
                        ...
                        <div className="flex-1 space-y-3">
                            ...
                            <p className="text-sm text-white/70">
                                Your ID verification needs to be renewed. Please
                                complete a new verification to continue using
                                creator features.
                            </p>
                            {gracePeriodEnd &&
                                verificationStatus.is_in_grace_period && (
                                    <p className="text-xs text-white/60">
                                        Grace period ends:{' '}
                                        {gracePeriodEnd.toLocaleDateString()}
                                    </p>
                                )}
                            {verificationStatus.is_expired &&
                                !verificationStatus.is_in_grace_period && (
                                    <p className="text-xs text-red-300">
                                        Your creator status has been disabled.
                                        Renew your verification to restore
                                        access.
                                    </p>
                                )}
                        </div>
    ...
    if (verificationStatus?.status === 'rejected') {
        return (
            <div className="rounded-3xl border border-red-400/20 bg-red-400/10 p-8">
                <div className="flex items-start gap-4">
                    <div className="flex size-12 items-center justify-center rounded-2xl border border-red-400/40 bg-red-400/20">
                    ...
                    <div className="flex-1 space-y-3">
                        ...
                        <p className="text-sm text-white/70">
                            Your verification was rejected. Please review your
    ...
    if (verificationStatus?.status === 'pending') {
        ...
        const getTimeInfo = () => {
            ...
            if (daysSinceCreated > 0) {
                return {
                    text: `${daysSinceCreated} day${daysSinceCreated > 1 ? 's' : ''}`,
                    minutes: minutesSinceCreated,
                };
            }
            if (hoursSinceCreated > 0) {
                return {
                    text: `${hoursSinceCreated} hour${hoursSinceCreated > 1 ? 's' : ''}`,
                    minutes: minutesSinceCreated,
                };
            }
            return {
                text: `${minutesSinceCreated} minute${minutesSinceCreated !== 1 ? 's' : ''}`,
                minutes: minutesSinceCreated,
            };
        };
        ...
        return (
            <div className="space-y-6">
                <div className="rounded-3xl border border-blue-400/20 bg-blue-400/10 p-8">
                    <div className="flex items-start gap-4">
                        <div className="flex size-12 items-center justify-center rounded-2xl border border-blue-400/40 bg-blue-400/20">
                            ...
                            {likelySubmitted ? (
                                ...
                                <div className="space-y-3">
                                    <p className="text-sm leading-relaxed text-white/80">
                                        Your verification has been submitted and
                                        is currently being reviewed by our
                                        security team. This process typically
                                    ...
                                    <div className="rounded-xl border border-blue-400/20 bg-blue-400/5 p-4">
                                        <p className="text-xs leading-relaxed text-blue-200/90">
                                            ...
                                            Our automated system will review
                                            your documents and identity
                                            verification. You'll receive an
                                            email notification once the review
                                            is complete.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3">
                                    <p className="text-sm leading-relaxed text-white/80">
                                        It looks like you may have started the
                                        verification process but didn't complete
                                        it. Don't worry—this happens sometimes
                                        if the verification window was closed or
                                    ...
                                    <div className="rounded-xl border border-violet-400/20 bg-violet-400/10 p-4">
                                        <p className="text-xs leading-relaxed text-violet-200/90">
                                            ...
                                            and complete your verification. The
                                ...
                                <div className="space-y-3">
                                    <p className="text-sm leading-relaxed text-white/80">
                                        Your verification is currently in
                                        progress. This typically takes just a
                                    ...
                                    {minutesSinceCreated >= 10 && (
                                        <div className="rounded-xl border border-amber-400/20 bg-amber-400/10 p-4">
                                            <p className="text-xs leading-relaxed text-amber-200/90">
                                                ...
                                                verification steps yet, make
                                                sure to finish the process in
                ...
                {probablyIncomplete && (
                    <Button
                        onClick={createSession}
                        disabled={creatingSession || loading}
                        className="w-full rounded-full bg-gradient-to-r from-violet-500 to-purple-600 px-8 py-6 text-lg font-semibold text-white shadow-[0_20px_50px_-20px_rgba(124,58,237,0.5)] transition hover:scale-[1.01] hover:shadow-[0_25px_60px_-20px_rgba(124,58,237,0.6)] disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:scale-100"
                    >
                        {creatingSession || loading ? (
                            <>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-white/5 via-white/5 to-violet-500/10 p-8 backdrop-blur-sm">
                <div className="space-y-6">
                    <div className="flex items-start gap-4">
                        <div className="flex size-12 items-center justify-center rounded-2xl border border-violet-400/40 bg-gradient-to-br from-violet-400/30 to-violet-500/20 p-3">
                        ...
                        <div className="flex-1 space-y-4">
                            <div>
                                ...
                                <p className="text-base leading-relaxed font-medium text-white/90">
                                    To become a creator and start earning,
                                    you'll need to verify your identity. The
                                    process is quick and secure, taking just a
                                    few minutes.
                                </p>
                            </div>
                            <div className="space-y-3 rounded-xl border border-white/10 bg-white/5 p-5">
                                ...
                                <ul className="space-y-3 text-sm text-white/80">
                                    <li className="flex items-start gap-3">
                                        <div className="mt-0.5 flex size-6 flex-shrink-0 items-center justify-center rounded-lg border border-violet-400/40 bg-violet-400/20">
                                        ...
                                        <span className="pt-0.5">
                                            ...
                                            ready (driver's license, passport,
                                            or national ID)
                                        </span>
                                    </li>
                                    <li className="flex items-start gap-3">
                                        ...
                                        <span className="pt-0.5">
                                            Take a quick selfie to confirm your
                            ...
                            <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/10 p-4">
                                <p className="text-xs leading-relaxed text-emerald-200/90">
                                    ...
                                    All information is processed by Sumsub with
                                    bank-level encryption. We never see your
                                    documents—only that your verification was
                                    successful. Your data is protected by GDPR,
                    ...
                    <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="mt-1 text-xs text-blue-200/80">
                                ...
                                window. If the window didn't open, please check
                                your popup blocker settings.
                            </p>
                        </div>
                    </div>
                </div>
